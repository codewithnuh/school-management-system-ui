...
export const useFetchTimeTables = (
  classId: number | "",
  sectionId: number | ""
): UseQueryResult<WeeklyTimetableData, Error> => { // Use the correct type for the data array
  return useQuery<WeeklyTimetableData, Error>({
    ...
    enabled: typeof classId === 'number' && typeof sectionId === 'number' && classId > 0 && sectionId > 0,
    staleTime: 5 * 60 * 1000,
    retry: 2,
  });
};
